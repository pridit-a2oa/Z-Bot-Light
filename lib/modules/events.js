...
class ZBotEvents {
  ...
  constructor(token, options) {
    ...
    this._zbot.bot.on(Events.ChannelCreate, async (channel) => {
      this.onChannelCreate(channel);
    });
    this._zbot.bot.on(Events.ChannelDelete, async (channel) => {
      this.onChannelDelete(channel);
    });
    this._zbot.bot.on(Events.ChannelPinsUpdate, async (channel, date) => {
      this.onChannelPinsUpdate(channel, date);
    });
    this._zbot.bot.on(Events.ChannelUpdate, async (oldChannel, newChannel) => {
      this.onChannelUpdate(oldChannel, newChannel);
    });
    this._zbot.bot.on(Events.Debug, async (string) => {
      this.onDebug(string);
    });
    this._zbot.bot.on(Events.ShardDisconnect, async () => {
      this.onDisconnect();
    });
    this._zbot.bot.on(Events.Error, async (error) => {
      this.onError(error);
    });
    this._zbot.bot.on(Events.GuildBanAdd, async (guild, user) => {
      this.onGuildBanAdd(guild, user);
    });
    this._zbot.bot.on(Events.GuildBanRemove, async (guild, user) => {
      this.onGuildBanRemove(guild, user);
    });
    this._zbot.bot.on(Events.GuildDelete, async (guild) => {
      this.onGuildCreate(guild);
    });
    this._zbot.bot.on(Events.GuildDelete, async (guild) => {
      this.onGuildDelete(guild);
    });
    this._zbot.bot.on(Events.GuildEmojiCreate, async (emoji) => {
      this.onGuildEmojiCreate(emoji);
    });
    this._zbot.bot.on(Events.GuildEmojiDelete, async (emoji) => {
      this.onGuildEmojiDelete(emoji);
    });
    this._zbot.bot.on(Events.GuildEmojiUpdate, async (oldEmoji, newEmoji) => {
      this.onGuildEmojiUpdate(oldEmoji, newEmoji);
    });
    this._zbot.bot.on(Events.GuildMemberAdd, async (member) => {
      this.onGuildMemberAdd(member);
    });
    this._zbot.bot.on(Events.GuildMemberAvailable, async (member) => {
      this.onGuildMemberAvailable(member);
    });
    this._zbot.bot.on(Events.GuildMemberRemove, async (member) => {
      this.onGuildMemberRemove(member);
    });
    this._zbot.bot.on(Events.GuildMembersChunk, async (members) => {
      this.onGuildMembersChunk(members);
    });
    ...
    this._zbot.bot.on(Events.GuildUnavailable, async (guild) => {
      this.onGuildUnavailable(guild);
    });
    this._zbot.bot.on(Events.GuildUpdate, async (oldGuild, newGuild) => {
      this.onGuildUpdate(oldGuild, newGuild);
    });
    this._zbot.bot.on(Events.MessageCreate, async (message) => {
      this.onMessage(message);
    });
    this._zbot.bot.on(Events.MessageDelete, async (message) => {
      this.onMessageDelete(message);
    });
    this._zbot.bot.on(Events.MessageBulkDelete, async (messages) => {
      this.onMessageDeleteBulk(messages);
    });
    this._zbot.bot.on(Events.MessageUpdate, async (oldMessage, newMessage) => {
      this.onMessageUpdate(oldMessage, newMessage);
    });
    this._zbot.bot.on(Events.PresenceUpdate, async (oldMember, newMember) => {
      this.onPresenceUpdate(oldMember, newMember);
    });
    this._zbot.bot.on(Events.ClientReady, async () => {
      this.onReady();
    });
    this._zbot.bot.on(Events.ShardReconnecting, async () => {
      this.onReconnecting();
    });
    this._zbot.bot.on(Events.GuildRoleCreate, async (role) => {
      this.onRoleCreate(role);
    });
    this._zbot.bot.on(Events.GuildRoleDelete, async (role) => {
      this.onRoleDelete(role);
    });
    this._zbot.bot.on(Events.GuildRoleUpdate, async (oldRole, newRole) => {
      this.onRoleUpdate(oldRole, newRole);
    });
    this._zbot.bot.on(Events.TypingStart, async (channel, user) => {
      this.onTypingStart(channel, user);
    });
    this._zbot.bot.on(Events.UserUpdate, async (oldUser, newUser) => {
      this.onUserUpdate(oldUser, newUser);
    });
    this._zbot.bot.on(Events.VoiceStateUpdate, async (oldMember, newMember) => {
      this.onVoiceStateUpdate(oldMember, newMember);
    });
    this._zbot.bot.on(Events.Warn, async (string) => {
      this.onWarn(string);
    });
  ...
  onGuildUnavailable(guild) {
    this.zbot.logBot(
      'Guild became unavailable: ' + guild.name + ' ( ' + guild.id + ' )'
    );
  }
  ...
  onReady() {
    this.zbot.logBot(
      `Ready to serve in ${this.zbot.bot.channels.cache.size} channels on ${this.zbot.bot.guilds.cache.size} servers, for a total of ${this.zbot.bot.users.cache.size} users.`
    );
  ...
  onReconnecting() {
    this._zbot.logBot(
      'Bot disconnected from discord API. Trying to reconnect.'
    );
  }
