...
class ZBotBERcon
{
   ...
   */
  bindChannels() {
    this.channels.command = this._guild.channels.cache.get(this.cfg.channels.commands);
    this.channels.side = this._guild.channels.cache.get(this.cfg.channels.side);
    this.channels.group = this._guild.channels.cache.get(this.cfg.channels.group);
    this.channels.direct = this._guild.channels.cache.get(this.cfg.channels.direct);
    this.channels.admin = this._guild.channels.cache.get(this.cfg.channels.admin);
    this.channels.connect = this._guild.channels.cache.get(this.cfg.channels.joins);
    this.channels.vehicle = this._guild.channels.cache.get(this.cfg.channels.vehicle);
    this.channels.default = this._guild.channels.cache.get(this.cfg.channels.default);
    this.channels.global = this._guild.channels.cache.get(this.cfg.channels.global);
  ...
  loginAction(err, success) {
    if (err) {
      ...
      if(this._loginAttempts < this._data.config.loginAttempts) {
        ...
        Helper.sendTextMessage( this.channels.default, this._cfg.name + ': Failed ' + this._loginAttempts + ' times to connect to the gameserver, please check if the server is online. <@&625328608545734675>');
  ...
  chatMessagesAction(message) {
    ...
    switch (true) {
      ...
      case (message.startsWith('Player #') || message.startsWith('Verified GUID') ):
        if(message.endsWith('disconnected') || message.includes('has been kicked')) {
          styledMessage = '```diff\n- ' + styledMessage + '\n```';
        } else {
    ...
    if (Array.isArray(message.match(/\b(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\.(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\b/))) {
      disableShow = true;
    }
