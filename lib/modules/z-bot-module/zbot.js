...
class ZBot
{
  ...
  welcomeClientAction(member) {
    // let channel = member.guild.channels.cache.find(val => val.name === this._cfg.specific.channels.welcome);
  ...
  leaveClientAction(member) {
    // let channel = member.guild.channels .find(val => val.name === this._cfg.specific.channels.goodbye);
  ...
  setLogChannel() {
    ...
    if (guilds) {
      let logguild = guilds.cache.find(val => val.name === this._cfg.discordServerName);
      if(logguild) {
   ...
   */
  encryptString(text) {
    return crypto.AES.encrypt(text, this._cfg.encryptionKey);
  }
  ...
  decryptString(encryptedText) {
    let bytes  = crypto.AES.decrypt(ciphertext.toString(), this._cfg.encryptionKey);
